...
## 5. Opening book content
`books/veltrix.bin` is generated entirely by the project itself
(`tools/build_opening_book.py`): hand-curated main lines plus engine
self-analysis verified through its own MultiPV search. No third-party game
data is shipped. The builder *optionally* merges external ECO opening TSV
files that YOU supply via `--eco-tsv` (e.g. the lichess-org/chess-openings
corpus, released under CC0/public domain); such files are inputs you provide,
