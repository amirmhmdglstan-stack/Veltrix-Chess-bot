...
# Filled glyph variants (black set) used for BOTH colours so a piece is always
# a solid silhouette; the body fill + offset outline stamps give the colour.
FILLED_PIECES = {k: UNICODE_PIECES[k.lower()] for k in UNICODE_PIECES}
WHITE_BODY, WHITE_LINE = "#fdfdfd", "#202020"
BLACK_BODY, BLACK_LINE = "#1e1e1e", "#f0f0f0"
FILES = "abcdefgh"
...
class BoardCanvas(tk.Canvas):
    ...
    def sq_to_xy(self, sq):
        f, r = sq % 8, sq // 8
        if self.flipped:
            # true 180-degree rotation, matching the square loop in redraw():
            # board file/right-mirrored, canvas row IS the board rank index
            f, r = 7 - f, r
        else:
            r = 7 - r
        return f, r
    ...
    def xy_to_sq(self, x, y):
        ...
        if not (0 <= f < 8 and 0 <= r < 8):
            return None
        if self.flipped:
            f = 7 - f          # rank index already correct for the rotation
        else:
            r = 7 - r
        return r * 8 + f

    def set_size(self, px):
        """Apply a board size explicitly (dialog / config).

        Relying on winfo_width() inside redraw is racy: the widget may still
        report the old geometry right after configure(), which made size
        changes a no-op. The requested size is now the single source of truth.
        """
        self.base_size = max(8 * 24, int(px))
        self.config(width=self.base_size, height=self.base_size)
        self.redraw()

    def cell(self):
        return max(24, int(getattr(self, "base_size", 480)) // 8)
    ...
    def redraw(self):
        ...
        # pieces (two-tone: solid body + contrasting outline stamps)
        for sq in range(64):
            p = self.board.board[sq]
            if p == ".":
                continue
            f, r = self.sq_to_xy(sq)
            self._piece_items(f * s + s / 2, r * s + s / 2 + s * 0.03, p, s,
                              tags=("piece", f"pc{sq}"))
    ...
    def _piece_items(self, x, y, piece, s, tags):
        """Draw one piece as a solid glyph with a contrasting outline.

        Both colours use the *filled* glyph shapes; white pieces get a white
        body with a dark outline, black pieces a black body with a light
        outline (the user's explicit request: pieces stay readable on any
        square colour). The outline is faked with 8 offset stamps behind the
        body. Returns the created item ids (outline stamps first, body last).
        """
        fs = max(12, int(s * 0.72))
        body = WHITE_BODY if piece.isupper() else BLACK_BODY
        line = WHITE_LINE if piece.isupper() else BLACK_LINE
        d = max(1, int(s * 0.022))
        font = ("Segoe UI Symbol", fs)   # falls back gracefully off-Windows
        glyph = FILLED_PIECES[piece]
        items = []
        for dx in (-d, 0, d):
            for dy in (-d, 0, d):
                if dx == 0 and dy == 0:
                    continue
                items.append(self.create_text(x + dx, y + dy, text=glyph,
                                              font=font, fill=line, tags=tags))
        items.append(self.create_text(x, y, text=glyph, font=font,
                                      fill=body, tags=tags))
        return items
    ...
    def _animate(self, old_board, move: Move):
        ...
        try:
            ...
            # same two-tone renderer as the static board: outline stamps are
            # the first N-1 items, body is last; the whole group slides.
            group = self._piece_items(x0, y0, piece, s, tags=("anim",))
            offs = [(self.coords(it)[0] - x0, self.coords(it)[1] - y0)
                    for it in group]
            for it in group:
                self.tag_raise(it)
            ...
            def step(i):
                ...
                for it, (ox, oy) in zip(group, offs):
                    self.coords(it, cx + ox, cy + oy)
