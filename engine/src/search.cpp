...
struct SearchThread {
    ...
    NNUE::Accumulator nnueAcc[MAX_PLY + 16];   // per-ply NNUE accumulators
...
// ------------------------------------------------- NNUE-aware eval plumbing --
inline bool use_nnue() { return gCfg.useNnue && NNUE::loaded(); }

// static evaluation for the node's accumulator (NNUE) or the handcrafted eval
inline Value eval_pos(SearchThread& th, int ply) {
    return use_nnue() ? NNUE::evaluate(th.nnueAcc[ply], th.pos.side_to_move())
                      : Eval::evaluate(th.pos);
}

// do_move keeping the per-ply NNUE accumulator in sync (legal-application
// sites only; siblings always re-push from the parent's accumulator so a
// wasted push on an illegal pseudo-move is harmless)
inline void tracked_do_move(SearchThread& th, SearchStack* ss, Move m) {
    if (use_nnue()) {
        const int ply = int(ss - th.stack);
        Color kside = NO_COLOR;
        const bool kingMove = NNUE::needs_king_refresh(th.pos, m, kside);
        NNUE::push(th.nnueAcc[ply], th.nnueAcc[ply + 1], th.pos, m);
        th.pos.do_move(m);
        if (kingMove) NNUE::refresh_side(th.nnueAcc[ply + 1], th.pos, kside);
    } else {
        th.pos.do_move(m);
    }
}
...
Value qsearch(SearchThread& th, SearchStack* ss, Value alpha, Value beta, int depth) {
    ...
    if (ply >= MAX_PLY) return eval_pos(th, ply);
    ...
    gTT.store(key, bestMove, to_tt_score(bestValue, ply),
              I16(rawStatic == VALUE_NONE ? eval_pos(th, ply) : rawStatic), 0, bnd);
    return bestValue;
}
...
Value negamax(SearchThread& th, SearchStack* ss, Value alpha, Value beta, int depth,
    ...
    else staticEval = eval_pos(th, ply);
    ...
    if (ply > 0 && !pvNode && !inCheck && excludedMove == MOVE_NONE) {
        ...
        if (depth >= 3 && eval >= beta && beta < VALUE_MATE_IN_MAX_PLY &&
            ...
            if (use_nnue()) th.nnueAcc[ply + 1] = th.nnueAcc[ply]; // position unchanged
            pos.do_null_move();
    ...
    if (excludedMove == MOVE_NONE) {
        ...
        Value evalOut = (staticEval == VALUE_NONE) ? eval_pos(th, ply) : staticEval;
...
void id_loop(SearchThread& th, bool mainThread) {
    ...
    // NNUE: build the root accumulators once per search position
    if (use_nnue()) NNUE::refresh(th.nnueAcc[0], pos);
